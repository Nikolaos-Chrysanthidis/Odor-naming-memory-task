...
figPATH = '/home/nik/BCPNNSim-olfaction/works/apps/olflang/Figures/DualNet/Network15x15/Semantization/SimpsonIndex_4Clusters/OdorCueNoise/'
...
def calc_recallwinner(cued_trpats,cue_trpats,cued_data,cue_data,recall_start_time,cued_net = 'LTM1'):
	...
	for i,timestep in enumerate(range(recall_start_time, cued_data.shape[1])):
		...
		cue_number = int(i/(2*(recall_ngap+recall_nstep)))
		cue = cues[cue_number]
...
def cued_recall(argv,savef=0):
	...
	os.chdir('/home/nik/BCPNNSim-olfaction/works/build/apps/olflang/') #current
	#os.chdir('/home/nik/BCPNNSim-olfaction/works/build/apps/olflang/plot_5Aand2C/') #for figure 5A, right and figure 2C
	#os.chdir('/home/nik/BCPNNSim-olfaction/works/build/apps/olflang/plot_cnn/') #for figure 5A
	#os.chdir('/home/nik/BCPNNSim-olfaction/works/build/apps/olflang/plot-raster-example/') #for figure 2D (omission example)
	...
	if  (plot_mode == 'full'):
		...
		if cued_net == 'LTM1':
			gs_top= gridspec.GridSpec(ncols=1,nrows=3,hspace=0.05,height_ratios = [3,0.25,2],right = 0.9,bottom=0.55,left=0.1)
			gs_bottom = gridspec.GridSpec(ncols=1,nrows=3,wspace=0.05,height_ratios = [2,0.25,2],top=0.53,right=0.9,left=0.1)
			...
			gs_top= gridspec.GridSpec(ncols=1,nrows=3,hspace=0.05,height_ratios = [3,0.25,2],right = 0.9,bottom=0.55,left=0.1)
			gs_bottom = gridspec.GridSpec(ncols=1,nrows=3,wspace=0.05,height_ratios = [2,0.25,2],top=0.43,right=0.9,left=0.1)
			gs_topright = gridspec.GridSpec(ncols=1,nrows=1,wspace=0.05,left=0.91,bottom=0.55,top=0.785,right=0.95)
			gs_bottomright = gridspec.GridSpec(ncols=1,nrows=1,wspace=0.05,left=0.91,top=0.315,right=0.95)
			#gs_bottom = gridspec.GridSpec(3, 1)

			ax_ltm1main = fig.add_subplot(gs_top[3,0])   #Main LTM1 Act plot
		...
		for i,cue_id in enumerate(cues):
			...
			for j,(cue_pat_id,cued_pat_id) in enumerate(zip(min_err_pats_cue,min_err_pats_cued)):
				...
				ax_cuedCosDistLog.plot(x,1-pats_cued[cued_pat_id],c=cued_colors,linewidth=1.5,linestyle=cued_ls)#linestyles[int(cued_pat_id/cued_pats.shape[0])])
				ax_cueCosDistLog.plot(x,1-pats_cue[cue_pat_id],c=cue_colors,linewidth=1.5,linestyle=linestyles[int(cue_pat_id/cue_pats.shape[0])])
			...
			for k,item in enumerate(cuenet_winners[x_start:x_end]):
				...
				if np.isnan(item):
					...
					ax_cueCosDistLog.annotate(cue_labels[int(item)],xy=(plot_recall_start_time+ i*(recall_nstep+recall_ngap)+k,label_annot_yloc-0.3),annotation_clip=False,color='k',fontsize=10,backgroundcolor='#FAF9F6')
			...
			for k,item in enumerate(winners[x_start:x_end]):
				if np.isnan(item):
					...
					ax_cuedCosDistLog.annotate(cued_labels[int(item)],xy=(plot_recall_start_time+ i*(recall_nstep+recall_ngap)+k,-0.3),annotation_clip=False,color='k',fontsize=10,backgroundcolor='#FAF9F6') #cued_col_list[int(item)]
		...
		ax_cuedCosDistLog.axhline(y=1-recall_thresh,linestyle='--',c='k',linewidth=0.7)
		...
		ax_cuedCosDistLog.set_xticks([])
		#ax_cuedCosDistLog.axis('off')

		ax_cueCosDistLog.axhline(y=1-recall_thresh,linestyle='--',c='k',linewidth=0.7)
		...
		ax_cueCosDistLog.set_yticks(np.arange(0, 1.2, step=0.2))
		...
		#ax_ltm2main.axis('off')
		ax_ltm2main.set(frame_on=False)
		ax_ltm2main.tick_params( labelbottom='on')
		...
		if cueHCs>0:
			...
			#shade_cuedHCs(cuedHCs_lists2,plot_data2,plot_recall_start_time,M2,ax_ltm2main) #used before for shading activated MCs #change
